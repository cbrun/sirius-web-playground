...
/**
 * A tiny helper to build AQL service call expressions from type-safe Java
 * method references instead of hardcoded strings.
 * <p>
 * Why use it:
 * <ul>
 * <li>Refactoring friendly: IDE rename of the Java method updates call
 * sites.</li>
 * <li>Navigation: find usages works, you can jump to the service
 * implementation.</li>
 ...
 * How it works:
 * <ol>
 * <li>You pass a <b>serializable</b> method reference to one of the factory
 * methods {@code of0}, {@code of1}, {@code of2}, {@code of3} for instance
 * methods, or {@code ofStatic0..3} for static methods. The arity suffix equals
 * the number of AQL parameters after {@code self}.</li>
 * <li>The helper extracts the Java method name through the standard lambda
 * serialization hook (reading a {@link SerializedLambda}). No service is
 * invoked.</li>
 * <li>You then call {@link #aqlSelf(String...)} or
 * {@link #aql(String, String...)} to build the final AQL string, delegating to
 * {@code AQLUtils}.</li>
 * </ol>
 * <p>
 * Important: the arguments passed to {@code aqlSelf(...)} and
 * {@code aql(var, ...)} are AQL snippets, not Java values. For example use
 * {@code "'declaredName'"} for a string literal and
 * {@code ViewFormDescriptionConverter.NEW_VALUE} if it already returns an AQL
 * snippet.
 * <p>
 * When to use which call:
 * <ul>
 * <li>{@link #aqlSelf(String...)} when the receiver is {@code self}, for
 * expressions like {@code aql:self.myService(...)}.</li>
 * <li>{@link #aql(String, String...)} when you target another variable in the
 * AQL context, for example {@code aql:elt.myService(...)}.</li>
 * </ul>
 * <p>
 * Type inference: if the compiler says something like <i>The type X does not
 * define methodName(Object, Object, ...)</i>, add a type witness to the factory
 * so it can match the real signature. Examples:
 *
 * <pre>{@code
 ...
 * ServiceMethod.<LAQueryService, Element>of0(LAQueryService::isComponent).aqlSelf();
 * }
 * </pre>
 * <p>
 * Performance: this uses reflection once per reference at startup to read a
 * method name. The cost is negligible compared to normal init work.
 *
 * @author cbrun
 *
 */
public final class ServiceMethod {
	...
	/**
	 * the Java method name that will be called from AQL. Useful if you need the raw
	 * name.
	 *
	 * @return the name.
	 */
	 ...
	 * <p>
	 * Use when the receiver is {@code self}. Parameters are concatenated as-is.
	 * Pass zero parameters for {@code myService()}.
	 *
	 * @param params AQL parameter snippets, for example {@code "'declaredName'"} or
	 *               {@code someVar}
	 * @return A full AQL expression string
	 */
	public String aqlSelf(String... params) {
		if (params == null || params.length == 0) {
			return getSelfServiceCallExpression(this.name);
		}
		return getSelfServiceCallExpression(this.name, Arrays.asList(params));
	}

	/**
	 * Returns the AQL expression for calling a service without any parameter using
	 * <code>self</code> as the instance.<br>
	 * For instance: <code>aql:self.myService()</code>
	 *
	 * @param serviceName The name of the service to call.
	 * @return An AQL expression (<code>String</code>) expressing the call of the
	 *         given service.
	 */
	 ...
	 * using <code>self</code> as the instance.<br>
	...
	public static String getSelfServiceCallExpression(String serviceName, List<String> parameters) {
		return AQL_SELF + '.' + serviceName + '(' + String.join(",", parameters) + ')';
	}

	/**
	 * Returns the AQL expression for calling a service with one parameter using
	 * <code>self</code> as the instance.<br>
	 * For instance: <code>aql:self.myService(sysml::Package)</code>
	 *
	 * @param serviceName The name of the service to call.
	 * @param parameter   The unique parameter.
	 * @return An AQL expression (<code>String</code>) expressing the call of the
	 *         given service.
	 */
	 ...
	 * <p>
	 * Use when you need to target a specific variable in the AQL scope, for example
	 * {@code elt} or {@code container}. Pass zero parameters for
	 * {@code myService()}.
	 *
	 * @param var    the AQL variable name to call the service on, for example
	 *               {@code "elt"}
	 * @param params AQL parameter snippets
	 * @return A full AQL expression string
	 */
	public String aql(String var, String... params) {
		...
		if (params == null || params.length == 0) {
			return getServiceCallExpression(var, this.name);
		}
		return getServiceCallExpression(var, this.name, Arrays.asList(params));
	}

	/**
	 * Returns the AQL expression for calling a service without any parameter.
	 *
	 * @param var         The part of the expression specifying the variable on
	 *                    which the service should be called.
	 * @param serviceName The name of the service to call.
	 * @return An AQL expression (<code>String</code>) expressing the call of the
	 *         given service.
	 */
	public static String getServiceCallExpression(String var, String serviceName) {
		return getServiceCallExpression(var, serviceName, List.of());
	}
	...
	public static String getServiceCallExpression(String var, String serviceName, String parameter) {
		return getServiceCallExpression(var, serviceName, List.of(parameter));
	}
	...
	public static String getServiceCallExpression(String var, String serviceName, List<String> parameters) {
		return AQL + var + '.' + serviceName + '(' + String.join(",", parameters) + ')';
	}
	...
	/**
	 * Instance method with signature
	 * {@code R method(T self, P1 p1, P2 p2, P3 p3, P4 p4)}.
	 */
	...
	/**
	 * Instance method with signature
	 * {@code R method(T self, P1 p1, P2 p2, P3 p3, P4 p4, P5 p5)}.
	 */
	...
	/**
	 * Instance method with signature
	 * {@code R method(T self, P1 p1, P2 p2, P3 p3, P4 p4, P5 p5, P6 p6)}.
	 */
	// CHECKSTYLE:OFF
